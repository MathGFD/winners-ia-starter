...
description: >
  ...
  files to _Sistema/Wiki-LLM/raw/ and wants them ingested, says "process this source",
  "ingest this article", "I added something to _Sistema/Wiki-LLM/raw/", or wants to
...
2. If the user says "process new sources" or similar, detect unprocessed files:
   - List all files in `_Sistema/Wiki-LLM/raw/` (excluding `_Sistema/Wiki-LLM/raw/assets/`)
   - Read `_Sistema/Wiki-LLM/wiki/log.md` and extract all previously ingested source filenames from `ingest` entries
   - Any file in `_Sistema/Wiki-LLM/raw/` not listed in the log is unprocessed
...
Create a new file in `_Sistema/Wiki-LLM/wiki/sources/` named after the source (slugified). Include:
...
- Create a new page in the appropriate subdirectory:
  - `_Sistema/Wiki-LLM/wiki/entities/` for people, organizations, products, tools
  - `_Sistema/Wiki-LLM/wiki/concepts/` for ideas, frameworks, theories, patterns
...
### 6. Update _Sistema/Wiki-LLM/wiki/index.md
...
### 7. Update _Sistema/Wiki-LLM/wiki/log.md

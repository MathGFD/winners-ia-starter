...
Read `_Sistema/Wiki-LLM/wiki/index.md` to identify relevant pages. Scan all category sections (Sources, Entities, Concepts, Synthesis) for entries related to the question.
...
```bash
qmd search "query terms" --path _Sistema/Wiki-LLM/wiki/
```
...
If the wiki pages don't fully answer the question, check relevant source summaries in `_Sistema/Wiki-LLM/wiki/sources/` for additional detail. Only go to files in `_Sistema/Wiki-LLM/raw/` as a last resort.
...
If the user agrees:
1. Create a new page in `_Sistema/Wiki-LLM/wiki/synthesis/` with proper frontmatter
2. Add an entry to `_Sistema/Wiki-LLM/wiki/index.md` under Synthesis
3. Append to `_Sistema/Wiki-LLM/wiki/log.md`: `## [YYYY-MM-DD] query | Question summary`

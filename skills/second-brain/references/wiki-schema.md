...
- **_Sistema/Wiki-LLM/raw/** — immutable source documents. The LLM reads from here but NEVER modifies these files.
- **_Sistema/Wiki-LLM/wiki/** — the LLM's workspace. Create, update, and maintain all files here.
- **_Sistema/Wiki-LLM/output/** — reports, query results, and generated artifacts go here.

Wiki subdirectories:
- `_Sistema/Wiki-LLM/wiki/sources/` — one summary page per ingested source
- `_Sistema/Wiki-LLM/wiki/entities/` — pages for people, organizations, products, tools
- `_Sistema/Wiki-LLM/wiki/concepts/` — pages for ideas, frameworks, theories, patterns
- `_Sistema/Wiki-LLM/wiki/synthesis/` — comparisons, analyses, cross-cutting themes

Two special files:
- `_Sistema/Wiki-LLM/wiki/index.md` — master catalog of every wiki page, organized by category. Update on every ingest.
- `_Sistema/Wiki-LLM/wiki/log.md` — append-only chronological record. Never edit existing entries.
...
When the user adds a file to _Sistema/Wiki-LLM/raw/ and asks you to process it:

1. Read the source completely
2. Discuss key takeaways with the user
3. Create a source summary page in `_Sistema/Wiki-LLM/wiki/sources/` with: title, source metadata, key claims, and a structured summary
...
6. Update `_Sistema/Wiki-LLM/wiki/index.md` with any new pages
7. Append to `_Sistema/Wiki-LLM/wiki/log.md`: `## [YYYY-MM-DD] ingest | Source Title`
...
1. Read `_Sistema/Wiki-LLM/wiki/index.md` to find relevant pages
2. Read the relevant wiki pages
3. Synthesize an answer with `[[wikilink]]` citations to wiki pages
4. If the answer produces a valuable artifact (comparison, analysis, new connection), offer to save it as a new page in `_Sistema/Wiki-LLM/wiki/synthesis/`
...
Each entry in `_Sistema/Wiki-LLM/wiki/index.md` is one line:
...
Each entry in `_Sistema/Wiki-LLM/wiki/log.md`:
...
- Source pages: `_Sistema/Wiki-LLM/wiki/sources/article-title-here.md` → `# Article Title Here`
- Entity pages: `_Sistema/Wiki-LLM/wiki/entities/entity-name.md` → `# Entity Name`
- Concept pages: `_Sistema/Wiki-LLM/wiki/concepts/concept-name.md` → `# Concept Name`
- Synthesis pages: `_Sistema/Wiki-LLM/wiki/synthesis/comparison-topic.md` → `# Comparison Topic`
...
1. **Download images locally.** In Obsidian Settings → Files and links, set "Attachment folder path" to `_Sistema/Wiki-LLM/raw/assets/`. Then use "Download attachments for current file" (bind it to a hotkey like Ctrl+Shift+D) after clipping an article.
2. **Reference images from wiki pages** using standard markdown: `![description](../_Sistema/Wiki-LLM/raw/assets/image-name.png)`. Keep the image in `_Sistema/Wiki-LLM/raw/assets/` — never copy images into `_Sistema/Wiki-LLM/wiki/`.
...
1. Never modify files in `_Sistema/Wiki-LLM/raw/`. They are immutable source material.
2. Always update `_Sistema/Wiki-LLM/wiki/index.md` when you create or delete a page.
3. Always append to `_Sistema/Wiki-LLM/wiki/log.md` when you perform an operation.
...
10. Keep `_Sistema/Wiki-LLM/wiki/index.md` concise — one line per page, under 120 characters per entry.
